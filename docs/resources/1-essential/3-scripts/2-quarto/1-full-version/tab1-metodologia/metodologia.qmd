...
ruta_nextflow_results <- file.path(ruta_bulk_rna_seq_pipeline, "resources", "2-nextflow-results")
ruta_proyecto_relativa <- file.path(ruta_bulk_rna_seq_pipeline, "resources", "2-nextflow-results", "1-project-data", nombre_proyecto)


# Rutas de resources
ruta_archives <- file.path(ruta_bulk_rna_seq_pipeline, "resources", "1-essential", "2-archives")
...
<ul>
  <li><strong>project_path</strong> &rArr; ruta que contiene los resultados del análisis completo realizado con <strong>miARma‑Seq</strong>.</li>
...
<p>
  Los datos del informe provienen de un análisis integral realizado con <strong>miARma‑Seq</strong> (<em>Eduardo Andrés-León et al.</em>), que procesa los archivos <code>FASTQ</code> desde la fase de control de calidad hasta la cuantificación de genes y análisis diferencial, incluyendo mRNA, miRNA y circRNA. Esta herramienta genera reportes intermedios y finales de cada etapa, permitiendo un flujo de trabajo completo y reproducible.
</p>
...
<p>
  A continuación se detalla la organización principal del directorio raíz del pipeline (<code>1-bukl-rna-seq</code>). 
...
<p>
  ...
  <a href="https://github.com/adrichez/GenoScribe/blob/main/6-info/1_BULK_RNA_SEQ_GUIDE.md" target="_blank" class="normalink">GenoScribe: Guía del Pipeline para Bull RNA-Seq</a>.
</p>
